...
async def update_user_endpoint(user_id:int, user_update:UserSQL, session:AsyncSession = Depends(get_db_session)):
...
async def convert_user_to_premium(user_id: int, session: AsyncSession = Depends(get_db_session)):
